...
\begin{example}
In \textbf{logistic regression},
$$\Pr(y=0)=[1+exp(-a^Tx)]^{-1}$$
$$\Pr(y=1)=[1+exp(-a^Tx)]^{-1}exp(-a^Tx)$$
So the likelihood function is:
$$\mathcal{L}= \prod_{i=1}^{n}{[1+exp(-a^Tx_i)]^{-y_i}[1+exp(a^Tx_i)]^{(y_i-1)}}$$
where $y_i \in \{0,1\}$.
Take the logarithm of the likelihood:
$$log(\mathcal{L})=\sum_{i=1}^n{-y_ilog(1+exp(-a^Tx_i))+(y_i-1)log(1+exp(a^Tx_i))}$$
...
$$\sum_{x=0}^\infty{f(x)}=\sum_{x=0}^\infty{e^{-\lambda} \frac{\lambda^x }{x!}}=e^{-x}\sum_{x=0}^\infty{\frac{\lambda^x}{x!}}=e^{-x}e^x=1$$
...
for $x=0,1,2,...,n$, where ${n \choose x} = \frac{n!}{x!(n-x)!}$ is the binomial coefficient. It can be interpreted that the probability of exact $k$ successes after $n$ trials.
...
Also, we can define ${r\choose k}$ when $r$ is a real number and $k$ is a integer:
\[{r\choose k} = \left\{\begin{array}{cc}
\frac{r(r-1)\dots(r-k+1)}{k!} & k\ge 0 \\
0 & k<0
\end{array}
\right.
\]
where ${r\choose 0}=1, {r\choose 1}=r$.
Then we can get a new binomial theorem:
$(1+z)^r =\frac{f(0)}{0!}z^0+\frac{f^{'}(0)}{1!}z^1+\dots=\sum_k{\frac{f^{(k)}(0)}{k!}z^k}=\sum_{k}{r \choose k}z^k$, $|z|<1$.
...
\item
If $nt>-1$, show that $(1-t)^n\ge 1-nt$.
\item 
If $-x<n<m$, show that $(1+\frac{x}{n})^n\le (1+\frac{x}{m})^m$.
...
A random variable $X$ that is gamma-distributed is denoted by
$X\sim \mbox{Gamma}(r,\frac{\alpha}{2})$,
$$
...
A random variable $X$ that is inverse gamma-distributed is denoted by $X\sim \mbox{Inv-Gamma}(r,\frac{\beta}{2})$,
$$
...
\item
$u\rightarrow0, K_r(u) \sim \begin{cases}
   \frac{1}{2}\Gamma(r)\left(\frac{u}{2}\right)^{-r} & r>0 \\
   \ln{u} & r=0
  \end{cases}$
\item
...
\textbf{Inverse Gaussian Distribution}

Specially, when $r=-\frac{1}{2}$,
\begin{eqnarray*}
f(x) &=& \left(\frac{\beta}{2\pi}\right)^{\frac{1}{2}}\exp({\sqrt{\alpha\beta}})x^{-\frac{3}{2}}\exp(-\frac{\alpha x+\beta x^{-1}}{2})\\
...
$$f(x) = \left(\frac{\alpha}{2\pi}\right)^{\frac{1}{2}}\exp({\sqrt{\alpha\beta}})x^{-\frac{1}{2}}\exp(-\frac{\alpha x+\beta x^{-1}}{2})$$
Note that $\int_0^\infty{x^{-\frac{1}{2}}\exp(-\frac{\alpha x+\beta x^{-1}}{2})dx}={(\frac{2\pi}{\alpha})}^{\frac{1}{2}}\exp{(-\sqrt{\alpha\beta})}$, because $\int_0^\infty{x^{-\frac{1}{2}}\exp(-\frac{a^2x+b^2x^{-1}}{2})dx}=\frac{\sqrt{2\pi}}{a}\exp{(-|ab|)}$. 
...
$$
where $\nu$ denotes the degree of freedom. When $\nu=1$, it is Cauchy distribution:
$$f(x)=\frac{1}{\pi\sigma}[1+(\frac{x-\mu}{\sigma})^2]^{-1}$$ 
and when $\nu\rightarrow\infty$, it is Gaussian distribution:
$$\lim_{\nu\rightarrow\infty}(1+\frac{\nu+1}{2\nu}\frac{2}{\nu+1}(\frac{x-\mu}{\sigma})^2)^{-\frac{\nu+1}{2}}=\exp(-\frac{(x-\mu)^2}{2\sigma^2}) $$
We can prove the Stirling Formula 
$\lim_{p\rightarrow\infty}\frac{\Gamma(p)}{(2\pi)^{\frac{1}{2}}p^{p-\frac{1}{2}}e^{-p}}=1$.
$$\lim_{\nu\rightarrow\infty}\nu^{-\frac{1}{2zx}}\frac{\Gamma(\frac{\nu+1}{2})}{\Gamma(\frac{\nu}{2})}$$
